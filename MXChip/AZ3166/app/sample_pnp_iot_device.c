...
typedef struct SAMPLE_CONTEXT_STRUCT {
  ...
  /* Generally, IoTHub Client and DPS Client do not run at the same time, user
     can use union as below to share the memory between IoTHub Client and DPS
     Client.

     NOTE: If user can not make sure sharing memory is safe, IoTHub Client and
     DPS Client must be defined seperately.  */
...
static UCHAR
    nx_azure_iot_tls_metadata_buffer[NX_AZURE_IOT_TLS_METADATA_BUFFER_SIZE];
...
static UINT exponential_backoff_with_jitter() {
  double jitter_percent =
      (SAMPLE_MAX_EXPONENTIAL_BACKOFF_JITTER_PERCENT / 100.0) *
      (rand() / ((double)RAND_MAX));
  UINT base_delay = SAMPLE_MAX_EXPONENTIAL_BACKOFF_IN_SEC;
  uint64_t delay;

  if (exponential_retry_count < (sizeof(UINT) * 8)) {
    delay = (uint64_t)((1 << exponential_retry_count) *
                       SAMPLE_INITIAL_EXPONENTIAL_BACKOFF_IN_SEC);
    if (delay <= (UINT)(-1)) {
...
static VOID exponential_backoff_reset() { exponential_retry_count = 0; }

static VOID connection_status_callback(NX_AZURE_IOT_PNP_CLIENT *hub_client_ptr,
                                       UINT status) {
  ...
  if (status) {
    ...
    tx_event_flags_set(&(sample_context.sample_events), SAMPLE_DISCONNECT_EVENT,
                       TX_OR);
  } else {
    printf("Connected to IoTHub.\r\n");
    tx_event_flags_set(&(sample_context.sample_events), SAMPLE_CONNECTED_EVENT,
                       TX_OR);
...
static VOID
message_receive_callback_properties(NX_AZURE_IOT_PNP_CLIENT *hub_client_ptr,
                                    VOID *context) {
...
static VOID
message_receive_callback_command(NX_AZURE_IOT_PNP_CLIENT *hub_client_ptr,
                                 VOID *context) {
  SAMPLE_CONTEXT *sample_ctx = (SAMPLE_CONTEXT *)context;

  NX_PARAMETER_NOT_USED(hub_client_ptr);
  tx_event_flags_set(&(sample_ctx->sample_events), SAMPLE_COMMAND_MESSAGE_EVENT,
                     TX_OR);
}

static VOID message_receive_callback_desired_property(
    NX_AZURE_IOT_PNP_CLIENT *hub_client_ptr, VOID *context) {
  SAMPLE_CONTEXT *sample_ctx = (SAMPLE_CONTEXT *)context;
...
static VOID sample_connect_action(SAMPLE_CONTEXT *context) {
  ...
  context->action_result = nx_azure_iot_pnp_client_connect(
      &(context->iotpnp_client), NX_FALSE, NX_WAIT_FOREVER);
  ...
  if (context->action_result == NX_AZURE_IOT_CONNECTING) {
    ...
    context->action_result = nx_azure_iot_pnp_client_properties_request(
        &(context->iotpnp_client), NX_WAIT_FOREVER);
  }
}
...
static VOID sample_disconnect_action(SAMPLE_CONTEXT *context) {
  ...
  context->action_result =
      nx_azure_iot_pnp_client_disconnect(&(context->iotpnp_client));
  context->state = SAMPLE_STATE_DISCONNECTED;
}
...
static VOID sample_initialize_iothub(SAMPLE_CONTEXT *context) {
  ...
  /* Run DPS.  */
  if ((status = sample_dps_entry(&(context->prov_client), &iothub_hostname,
                                 &iothub_hostname_length, &iothub_device_id,
  ...
  printf("IoTHub Host Name: %.*s; Device ID: %.*s.\r\n", iothub_hostname_length,
         iothub_hostname, iothub_device_id_length, iothub_device_id);

  /* Initialize IoTHub client.  */
  if ((status = nx_azure_iot_pnp_client_initialize(
           iotpnp_client_ptr, &nx_azure_iot, iothub_hostname,
           iothub_hostname_length, iothub_device_id, iothub_device_id_length,
           ...
           sizeof(nx_azure_iot_tls_metadata_buffer), &root_ca_cert))) {
    printf("Failed on nx_azure_iot_pnp_client_initialize!: error code = "
           "0x%08x\r\n",
           status);
    context->action_result = status;
    return;
  ...
  if ((status = nx_secure_x509_certificate_initialize(
           ...
           (USHORT)sample_device_cert_len, NX_NULL, 0,
           (UCHAR *)sample_device_private_key_ptr,
           (USHORT)sample_device_private_key_len, DEVICE_KEY_TYPE))) {
  ...
  else if ((status = nx_azure_iot_pnp_client_device_cert_set(
                iotpnp_client_ptr, &device_certificate))) {
    printf("Failed on nx_azure_iot_pnp_client_device_cert_set!: error code = "
           "0x%08x\r\n",
           status);
  }
#else
  ...
  if ((status = nx_azure_iot_pnp_client_symmetric_key_set(
           iotpnp_client_ptr, (UCHAR *)DEVICE_SYMMETRIC_KEY,
           sizeof(DEVICE_SYMMETRIC_KEY) - 1))) {
    printf("Failed on nx_azure_iot_pnp_client_symmetric_key_set! error: "
           "0x%08x\r\n",
           status);
  }
#endif /* USE_DEVICE_CERTIFICATE */
  ...
  /* Set connection status callback.  */
    ...
    printf("Failed on connection_status_callback!\r\n");
                  ...
                  iotpnp_client_ptr, NX_AZURE_IOT_PNP_PROPERTIES,
                  message_receive_callback_properties, (VOID *)context))) {
    printf("device properties callback set!: error code = 0x%08x\r\n", status);
  } else if ((status = nx_azure_iot_pnp_client_receive_callback_set(
                  iotpnp_client_ptr, NX_AZURE_IOT_PNP_COMMAND,
                  message_receive_callback_command, (VOID *)context))) {
    printf("device command callback set!: error code = 0x%08x\r\n", status);
  } else if ((status = nx_azure_iot_pnp_client_receive_callback_set(
                  iotpnp_client_ptr, NX_AZURE_IOT_PNP_DESIRED_PROPERTIES,
    ...
    printf("device desired property callback set!: error code = 0x%08x\r\n",
      ...
      (status = nx_azure_iot_pnp_client_component_add(
           iotpnp_client_ptr, (const UCHAR *)sample_device_component,
           sizeof(sample_device_component) - 1)) ||
      (status = nx_azure_iot_pnp_client_component_add(
           iotpnp_client_ptr, (const UCHAR *)sample_fallcurve_1_component,
           sizeof(sample_fallcurve_1_component) - 1)) ||
      (status = nx_azure_iot_pnp_client_component_add(
           iotpnp_client_ptr, (const UCHAR *)sample_fallcurve_2_component,
...
static VOID sample_trigger_action(SAMPLE_CONTEXT *context) {
  ...
  case SAMPLE_STATE_INIT: {
    tx_event_flags_set(&(context->sample_events), SAMPLE_INITIALIZATION_EVENT,
                       TX_OR);
  } break;
  ...
  case SAMPLE_STATE_CONNECTED: {
    if ((tx_time_get() - context->last_periodic_action_tick) >=
        (5 * NX_IP_PERIODIC_RATE)) {
      context->last_periodic_action_tick = tx_time_get();
      tx_event_flags_set(&(context->sample_events), SAMPLE_TELEMETRY_SEND_EVENT,
                         TX_OR);
      tx_event_flags_set(&(context->sample_events),
                         SAMPLE_DEVICE_REPORTED_PROPERTIES_EVENT, TX_OR);
    }
  } break;

  case SAMPLE_STATE_DISCONNECTED: {
    tx_event_flags_set(&(context->sample_events), SAMPLE_RECONNECT_EVENT,
                       TX_OR);
  } break;
  }
}
...
static VOID sample_command_action(SAMPLE_CONTEXT *sample_context_ptr) {
  ...
  if ((status = nx_azure_iot_pnp_client_command_receive(
           &(sample_context_ptr->iotpnp_client), &component_name_ptr,
           &component_name_length, &pnp_command_name_ptr,
           &pnp_command_name_length, &context_ptr, &context_length,
  ...
  if (component_name_ptr != NX_NULL) {
    printf("Received component: %.*s ", component_name_length,
           component_name_ptr);
  } else {
    printf("Received component: root component ");
  }

  printf("command: %.*s", pnp_command_name_length,
         (CHAR *)pnp_command_name_ptr);
  printf("\r\n");

  if ((status = nx_azure_iot_json_writer_with_buffer_init(
           &json_writer, scratch_buffer, sizeof(scratch_buffer)))) {
    printf("Failed to initialize json builder response \r\n");
    nx_azure_iot_json_reader_deinit(&json_reader);
    return;
  }

  if ((status = sample_pnp_device_process_command(
           &sample_device, (UCHAR *)component_name_ptr, component_name_length,
           (UCHAR *)pnp_command_name_ptr, pnp_command_name_length, &json_reader,
  ...
  } else if ((status = pnp_vt_process_command(
                  verified_telemetry_DB, &(sample_context_ptr->iotpnp_client),
    ...
    printf("Successfully executed command %.*s on of the Verified Telemetry "
           "components\r\n",
           pnp_command_name_length, pnp_command_name_ptr);
    response_length = nx_azure_iot_json_writer_get_bytes_used(&json_writer);
  } else {
  ...
  if ((status = nx_azure_iot_pnp_client_command_message_response(
           &(sample_context_ptr->iotpnp_client), status_code, context_ptr,
           context_length, scratch_buffer, response_length, NX_WAIT_FOREVER))) {
...
static VOID
sample_desired_properties_parse(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
  ...
  while (nx_azure_iot_pnp_client_desired_component_property_value_next(
             pnp_client_ptr, &copy_json_reader, message_type, &component_ptr,
             &component_len, &name_value_reader) == NX_AZURE_IOT_SUCCESS) {
    if (pnp_vt_process_property_update(
            verified_telemetry_DB, pnp_client_ptr, component_ptr, component_len,
            &name_value_reader, version) == NX_AZURE_IOT_SUCCESS) {
      printf(
          "property updated on one of the Verified Telemetry components\r\n");
    } else {
      if (component_ptr) {
  ...
  while (nx_azure_iot_pnp_client_reported_component_property_value_next(
             pnp_client_ptr, &copy_json_reader, message_type, &component_ptr,
             &component_len, &name_value_reader) == NX_AZURE_IOT_SUCCESS) {
    if (pnp_vt_process_reported_property_sync(
            verified_telemetry_DB, pnp_client_ptr, component_ptr, component_len,
            &name_value_reader, version) == NX_AZURE_IOT_SUCCESS) {
...
static VOID sample_device_desired_property_action(SAMPLE_CONTEXT *context) {
  ...
  if ((status = nx_azure_iot_pnp_client_desired_properties_receive(
           &(context->iotpnp_client), &json_reader, &desired_properties_version,
           NX_WAIT_FOREVER))) {
...
static VOID sample_device_reported_property_action(SAMPLE_CONTEXT *context) {
  ...
  if (sample_device_properties_sent == 0) {
    if ((status = sample_pnp_device_led_state_property(
             &sample_device, &(context->iotpnp_client)))) {
      printf("Failed sample_pnp_device_led_state_property: error code = "
             "0x%08x\r\n",
             status);
    } else {
  ...
  if ((status = pnp_vt_properties(verified_telemetry_DB,
...
static VOID sample_telemetry_action(SAMPLE_CONTEXT *context) {
  ...
  if ((status = sample_pnp_device_telemetry_send(&sample_device,
                                                 &(context->iotpnp_client))) !=
      NX_AZURE_IOT_SUCCESS) {
...
static UINT sample_dps_entry(NX_AZURE_IOT_PROVISIONING_CLIENT *prov_client_ptr,
  ...
  if ((status = nx_azure_iot_provisioning_client_initialize(
           prov_client_ptr, &nx_azure_iot, (UCHAR *)ENDPOINT,
           sizeof(ENDPOINT) - 1, (UCHAR *)ID_SCOPE, sizeof(ID_SCOPE) - 1,
  ...
  *iothub_device_id_length = sizeof(sample_iothub_device_id);
  ...
  /* Set device certificate.  */
  else if ((status = nx_azure_iot_provisioning_client_device_cert_set(
                prov_client_ptr, &device_certificate))) {
    printf("Failed on nx_azure_iot_provisioning_client_device_cert_set!: error "
           "code = 0x%08x\r\n",
           status);
  }
#else
  ...
  if ((status = nx_azure_iot_provisioning_client_symmetric_key_set(
           prov_client_ptr, (UCHAR *)DEVICE_SYMMETRIC_KEY,
           sizeof(DEVICE_SYMMETRIC_KEY) - 1))) {
  ...
  /* Register device */
  else if ((status = nx_azure_iot_provisioning_client_register(
                prov_client_ptr, NX_WAIT_FOREVER))) {
  ...
  /* Get Device info */
                ...
                prov_client_ptr, sample_iothub_hostname, iothub_hostname_length,
                sample_iothub_device_id, iothub_device_id_length))) {
    printf(
        "Failed on nx_azure_iot_provisioning_client_iothub_device_info_get!: "
        "error code = 0x%08x\r\n",
        status);
  } else {
 ...
 * |                        | |                         |   | | | | |   |
 * CONNECT           | CONNECTING             | |                         |   |
 * FAIL             |   FAIL                 | REINITIALIZE| RECONNECT|   | | |
 *             |                         |   |                   v |  DISCONNECT
 *             |                         |   |        +----------+-+ | | |   |
 * |            |                      | |                         | +------->+
 * |                      | |                         |            | DISCONNECT
...
static VOID sample_event_loop(SAMPLE_CONTEXT *context) {
  ...
  while (loop) {
    ...
    if (tx_event_flags_get(&(context->sample_events), SAMPLE_ALL_EVENTS,
                           TX_OR_CLEAR, &app_events, 5 * NX_IP_PERIODIC_RATE)) {
      if (context->state == SAMPLE_STATE_CONNECTED) {
...
static VOID log_callback(az_log_classification classification, UCHAR *msg,
                         UINT msg_len) {
...
VOID sample_entry(NX_IP *ip_ptr, NX_PACKET_POOL *pool_ptr, NX_DNS *dns_ptr,
  ...
  screen_clear();
  screen_print_without_erasing(" Verified", L0);
  screen_print_without_erasing(" Telemetry", L1);
  screen_print_without_erasing("   Sample", L2);
  screen_update();

  /* Create Azure IoT handler.  */
  if ((status = nx_azure_iot_create(
           &nx_azure_iot, (UCHAR *)"Azure IoT", ip_ptr, pool_ptr, dns_ptr,
  ...
  if ((status = nx_secure_x509_certificate_initialize(
           &root_ca_cert, (UCHAR *)azure_iot_root_ca,
           (USHORT)azure_iot_root_ca_len, NX_NULL, 0, NULL, 0,
  ...
  tx_event_flags_set(&(sample_context.sample_events),
                     SAMPLE_INITIALIZATION_EVENT, TX_OR);
