...
NX_AZURE_IOT_PNP_CLIENT *iotpnp_client_ptr = &(context -> iotpnp_client);
                                                                    ...
                                                                    (VOID *)context)))
    {
        printf("device desired property callback set!: error code = 0x%08x\r\n", status);
                                                             ...
                                                             (const UCHAR *)sample_device_component,
...
static VOID sample_command_action(SAMPLE_CONTEXT *sample_context_ptr)
{
    ...
    if ((status = sample_pnp_device_process_command(&sample_device, (UCHAR *)component_name_ptr, component_name_length,
    ...
    else if ((status = pnp_vt_process_command(verified_telemetry_DB, &(sample_context_ptr -> iotpnp_client), 
                                            ...
                                            UINT message_type, ULONG version)
{
    ...
    {
        if (pnp_vt_process_property_update(verified_telemetry_DB, pnp_client_ptr,
                                                        component_ptr, component_len,
...
VOID sample_entry(NX_IP *ip_ptr, NX_PACKET_POOL *pool_ptr, NX_DNS *dns_ptr, UINT (*unix_time_callback)(ULONG *unix_time), void **verified_telemetry_DB_shared)
{
    ...
    *verified_telemetry_DB_shared = verified_telemetry_DB;
    ...
    if ((status = nx_secure_x509_certificate_initialize(&root_ca_cert, (UCHAR *)azure_iot_root_ca,
