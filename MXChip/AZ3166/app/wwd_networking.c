...
#define SAMPLE_TX_POOL_SIZE                                                    \
  ((SAMPLE_PACKET_SIZE + sizeof(NX_PACKET)) * SAMPLE_TX_PACKET_COUNT)
#define SAMPLE_RX_POOL_SIZE                                                    \
  ((SAMPLE_PACKET_SIZE + sizeof(NX_PACKET)) * SAMPLE_RX_PACKET_COUNT)
...
static void print_address(CHAR *preable, ULONG address) {
  printf("\t%s: %d.%d.%d.%d\r\n", preable, (u_int8_t)(address >> 24),
         (u_int8_t)(address >> 16 & 0xFF), (u_int8_t)(address >> 8 & 0xFF),
...
static UINT dns_create() {
  ...
  /* Is the DNS client configured for the host application to create the packet
   * pool? */
#ifdef NX_DNS_CLIENT_USER_CREATE_PACKET_POOL
  status =
      nx_dns_packet_pool_set(&nx_dns_client, nx_ip.nx_ip_default_packet_pool);
  if (status) {
  ...
  nx_dhcp_interface_user_option_retrieve(
      &dhcp_client, 0, NX_DHCP_OPTION_DNS_SVR, (UCHAR *)(dns_server_address),
...
/* Join Network.  */
static UINT wifi_network_join(void *pools, CHAR *ssid, CHAR *password,
                              WiFi_Mode mode) {
  ...
  while (wwd_wifi_join(&wiced_ssid, wiced_security, (uint8_t *)password,
...
int platform_init(CHAR *ssid, CHAR *password, WiFi_Mode mode) {
  ...
  status = nx_packet_pool_create(&nx_pool[0], "NetX Main TX Packet Pool",
                                 SAMPLE_PACKET_SIZE, sample_tx_pool_stack,
                                 SAMPLE_TX_POOL_SIZE);
  ...
  status = nx_packet_pool_create(&nx_pool[1], "NetX Main RX Packet Pool",
                                 SAMPLE_PACKET_SIZE, sample_rx_pool_stack,
                                 SAMPLE_RX_POOL_SIZE);
  if (status != NX_SUCCESS) {
  ...
  /* Create an IP instance for the DHCP Client. The rest of the DHCP Client set
     up is handled by the client thread entry function.  */
  status = nx_ip_create(&nx_ip, "NetX IP Instance 0", SAMPLE_IPV4_ADDRESS,
                        SAMPLE_IPV4_MASK, &nx_pool[0],
                        wiced_sta_netx_duo_driver_entry,
                        (UCHAR *)sample_ip_stack, SAMPLE_IP_STACK_SIZE, 1);
  if (status != NX_SUCCESS) {
    nx_packet_pool_delete(&nx_pool[0]);
    nx_packet_pool_delete(&nx_pool[1]);
  ...
  status = nx_arp_enable(&nx_ip, (VOID *)sample_arp_cache_area,
