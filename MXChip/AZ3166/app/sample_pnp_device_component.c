...
/* Telemetry key */
static const CHAR telemetry_name_soilMoistureExternalRaw[] =
    "soilMoistureExternal";
static const CHAR telemetry_name_accelerometerXExternalRaw[] =
    "accelerometerXExternal";
...
static UINT sample_pnp_device_set_led_state_command(
    SAMPLE_PNP_DEVICE_COMPONENT *handle,
...
UINT get_sensor_data(SAMPLE_PNP_DEVICE_COMPONENT *handle) {
  ...
  UINT soilMoistureADCData = adc_read(&hadc1, ADC_CHANNEL_8);
...
UINT sample_pnp_device_process_command(
    SAMPLE_PNP_DEVICE_COMPONENT *handle, UCHAR *component_name_ptr,
    UINT component_name_length, UCHAR *pnp_command_name_ptr,
    UINT pnp_command_name_length, NX_AZURE_IOT_JSON_READER *json_reader_ptr,
  ...
  if (handle->component_name_length != component_name_length ||
      strncmp((CHAR *)handle->component_name_ptr, (CHAR *)component_name_ptr,
              component_name_length) != 0) {
    return (NX_NOT_SUCCESSFUL);
  }

  if (pnp_command_name_length != (sizeof(set_led_state) - 1) ||
      strncmp((CHAR *)pnp_command_name_ptr, (CHAR *)set_led_state,
              pnp_command_name_length) != 0) {
    printf("PnP command=%.*s is not supported on device component\r\n",
           pnp_command_name_length, pnp_command_name_ptr);
    dm_status = 404;
  } else {
    dm_status = (sample_pnp_device_set_led_state_command(
                     handle, json_reader_ptr, json_response_ptr) !=
                 NX_AZURE_IOT_SUCCESS)
...
UINT sample_pnp_device_telemetry_send(
  ...
  if ((status = nx_azure_iot_pnp_client_telemetry_message_create(
           iotpnp_client_ptr, handle->component_name_ptr,
           handle->component_name_length, &packet_ptr, NX_WAIT_FOREVER))) {
    printf("Telemetry message create failed!: error code = 0x%08x\r\n", status);
    return (status);
  }

  /* Build telemetry JSON payload */
  if (nx_azure_iot_json_writer_with_buffer_init(&json_writer, scratch_buffer,
  ...
  if (nx_azure_iot_json_writer_append_begin_object(&json_writer) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_soilMoistureExternalRaw,
          sizeof(telemetry_name_soilMoistureExternalRaw) - 1,
          handle->soilMoistureExternalRaw, DOUBLE_DECIMAL_PLACE_DIGITS) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_accelerometerXExternalRaw,
          sizeof(telemetry_name_accelerometerXExternalRaw) - 1,
          handle->accelerometerXExternalRaw, DOUBLE_DECIMAL_PLACE_DIGITS) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_sensorTemperature,
          sizeof(telemetry_name_sensorTemperature) - 1,
          handle->sensorTemperature, DOUBLE_DECIMAL_PLACE_DIGITS) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_sensorPressure,
          sizeof(telemetry_name_sensorPressure) - 1, handle->sensorPressure,
          DOUBLE_DECIMAL_PLACE_DIGITS) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_sensorHumidity,
          sizeof(telemetry_name_sensorHumidity) - 1, handle->sensorHumidity,
          DOUBLE_DECIMAL_PLACE_DIGITS) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_sensorAcceleration,
          sizeof(telemetry_name_sensorAcceleration) - 1,
          handle->sensorAcceleration, DOUBLE_DECIMAL_PLACE_DIGITS) ||
      nx_azure_iot_json_writer_append_property_with_double_value(
          &json_writer, (UCHAR *)telemetry_name_sensorMagnetic,
          sizeof(telemetry_name_sensorMagnetic) - 1, handle->sensorMagnetic,
          DOUBLE_DECIMAL_PLACE_DIGITS) ||
  ...
  if ((status = nx_azure_iot_pnp_client_telemetry_send(
           iotpnp_client_ptr, packet_ptr, (UCHAR *)scratch_buffer,
           buffer_length, NX_WAIT_FOREVER))) {
...
UINT sample_pnp_device_led_state_property(
  ...
  if ((status = nx_azure_iot_pnp_client_reported_properties_create(
           iotpnp_client_ptr, &json_writer, NX_WAIT_FOREVER))) {
  ...
  if ((status = nx_azure_iot_pnp_client_reported_property_component_begin(
           iotpnp_client_ptr, &json_writer, handle->component_name_ptr,
           handle->component_name_length)) ||
      (status = nx_azure_iot_json_writer_append_property_with_bool_value(
           &json_writer, (const UCHAR *)reported_led_state,
           sizeof(reported_led_state) - 1, handle->sensorLEDState)) ||
      (status = nx_azure_iot_pnp_client_reported_property_component_end(
           iotpnp_client_ptr, &json_writer))) {
  ...
  if ((status = nx_azure_iot_pnp_client_reported_properties_send(
           iotpnp_client_ptr, &json_writer, NX_NULL, &response_status, NX_NULL,
